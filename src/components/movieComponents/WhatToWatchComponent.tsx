...
const answers = ['Based on your favorites list, i think you should like this one ! ','Ok, how about this one ?','How about this one?','This one is really cool !','Something for you !','Perfect choice for you !']
    const selectMovie = async () => {
        ...
        if (userData.favMovies.filter(el => el.movie_id === whatToWatch.id).length > 0) {
    ...
    return (
        <div className="whatToWatch">
            ...
            {displayMovie ? proposedMovie ? <ProposedMovieComponent movie={proposedMovie}/> : <Spinner returnRoute={'/'}/> : null}
            <button onClick={handleFindMovie} className="goBack">{!displayMovie?'propose something...':'want something else?'}</button>
