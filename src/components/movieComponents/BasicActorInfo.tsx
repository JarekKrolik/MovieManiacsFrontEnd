...
export const BasicActorInfo = (props:Props)=>{
    ...
    const favList = obj.favActors
    const list = favList?.map(e => e.actor_id)


    return(
        <div className="basicInfo actor">
            <FavouriteIcon id={foundData.id} user={obj.name} type={'actor'} title={foundData.name} switchedOn={list?list.includes(foundData.id):false} image={foundData.image}/>
